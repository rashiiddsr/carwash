...
app.post('/expenses', asyncHandler(async (req, res) => {
  ...
  const { expense_date, amount, category, notes, employee_id } = req.body;

  if (!expense_date || amount === undefined || !category || !notes) {
    return res.status(400).json({ message: 'expense_date, amount, category, dan notes wajib diisi' });
  }
  ...
  if (category === 'KASBON') {
    ...
    const { startDate, endDate } = getWeekRange(expense_date);
    ...
    const [omzetRows] = await pool.query(
      `SELECT COALESCE(SUM(price), 0) AS total_omzet
       FROM transactions
       WHERE employee_id = ?
         AND trx_date BETWEEN ? AND ?`,
      [resolvedEmployeeId, startDate, endDate]
    ...
    const maxKasbon = Number(omzetRows[0]?.total_omzet || 0) * 0.3;
